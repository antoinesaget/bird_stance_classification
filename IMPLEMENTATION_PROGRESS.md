...
| C3 Label Studio deployment + annotation config | PARTIAL | Stack up (`postgres`, `label-studio`, `ml-backend`); `/Users/antoine/bird_leg/labelstudio/label_config.xml`; `/Users/antoine/bird_leg/docs/annotator_guide.md`; 50-image import bundle generated at `/Users/antoine/bird_leg/data/birds_project/labelstudio/imports/scolop2_sample50.tasks.json`; manual UI checks passed for persistence and speed after enabling "Use predictions to prelabel tasks" + disabling interactive preannotations; 2-column labeling layout validated | Intentional temporary deviation vs strict spec conditional UI: `activity`/`support`/`legs`/`resting_back` are always visible (optional) to avoid Label Studio per-region prefill persistence issues |
