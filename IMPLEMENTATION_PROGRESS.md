...
## Snapshot summary
- `DONE`: C0, C1, C2, C4, C5, C6, C7, C8, C9, C10
- `PARTIAL`: C3, C11
- `PENDING`: none
...
| C3 Label Studio deployment + annotation config | PARTIAL | Stack up (`postgres`, `label-studio`, `ml-backend`); `/Users/antoine/bird_leg/labelstudio/label_config.xml`; `/Users/antoine/bird_leg/docs/annotator_guide.md`; 50-image import bundle generated at `/Users/antoine/bird_leg/data/birds_project/labelstudio/imports/scolop2_sample50.tasks.json`; switched back to strict conditional UI (`readability/specie` gating + `legs` gating) with new schema (`specie`, `behavior`, `substrate`, `readability=readable/occluded/unreadable`) and 2-column layout | Manual validation still required on strict variant in live Label Studio project (prefill + persistence + speed comparison across variant files) |
