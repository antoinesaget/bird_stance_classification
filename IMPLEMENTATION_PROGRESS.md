...
- Manual validation update (2026-02-17): `manual_values_persist_after_update=PASS`, `deleted_predictions_stay_deleted=PASS` after disabling interactive preannotations. Remaining discrepancy `single_vs_label_all_consistent=FAIL` is currently isolated to Label Studio mode behavior (`Label All Tasks` annotation view vs single-task prediction view), not backend prefill generation.
