...
LINES_LIMIT="${LINES_LIMIT:-100}"
LINES_UNTOUCHED_ONLY="${LINES_UNTOUCHED_ONLY:-1}"
LINES_REPLACE_EXISTING="${LINES_REPLACE_EXISTING:-1}"

REPORT_OUT="$(resolve_repo_path "${LINES_PREDICTION_REPORT_OUT:-${LINES_DATA_ROOT:-/mnt/tank/media/lines_project}/${LINES_IMPORT_RELATIVE_ROOT}/${LINES_BATCH_NAME}.refresh-report.json}")"

log "Refreshing untouched predictions for project '$LINES_PROJECT_ID' from '$LINES_ML_BACKEND_URL'"
...
if [[ "$LINES_UNTOUCHED_ONLY" != "0" ]]; then
  ARGS+=(--untouched-only)
fi

if [[ "$LINES_REPLACE_EXISTING" != "0" ]]; then
  ARGS+=(--replace-existing)
fi

"$PYTHON_BIN" scripts/prefill_labelstudio_predictions.py "${ARGS[@]}"
